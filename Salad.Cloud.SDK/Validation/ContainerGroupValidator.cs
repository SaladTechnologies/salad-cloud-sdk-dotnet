...
public class ContainerGroupValidator : AbstractValidator<ContainerGroup?>
{
    public ContainerGroupValidator()
    {
        ...
        RuleFor(ContainerGroup => ContainerGroup.Replicas)
            ...
            .WithMessage("Minimum for replicas is 100.")
            .NotNull()
        ...
        RuleFor(ContainerGroup => ContainerGroup.Networking)
            .Custom(
                (containerGroupNetworking, context) =>
                {
                    if (containerGroupNetworking != null)
                    {
                        var validator = new ContainerGroupNetworkingValidator();
                        var result = validator.Validate(containerGroupNetworking);
                        if (!result.IsValid)
                        {
                            foreach (var failure in result.Errors)
                            {
